...
export default async function AutomationPage({ params }: { params: { id: string } }) {
  ...
  if (access.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] px-8 text-center bg-white border border-rose-100 rounded-[40px] shadow-2xl shadow-rose-200/10">
        ...
        <p className="mt-4 text-gray-500 font-medium max-w-sm">Organizácia nemá pridelenú automatizáciu: <span className="font-bold text-gray-800">"{module.name}"</span>. Kontaktujte administrátora.</p>
  ...
  const FormUI = module.ui.form;

  return (
    <div className="flex flex-col gap-12 pb-20">
      {/* 1. Main Dashboard View (Always at the top) */}
      <DashboardView module={module} tenantId={tenantId!} />

      {/* 2. Interactive Execution Layer (Composition Pattern) */}
      { (module.type === "manual" || module.type === "form") && FormUI && (
        <FormView id={module.id} tenantId={tenantId!} name={module.name}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-12 items-start">
        <LogsView module={module} tenantId={tenantId!} />
        <SettingsView module={module} tenantId={tenantId!} />
      </div>
    </div>
  );
}
